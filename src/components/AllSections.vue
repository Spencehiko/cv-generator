    ...
    <div v-for="(section, index) in sections" :key="index" class="px-10 py-4">
        <div class="flex flex-row border-b-2 pb-1">
            ...
            <div class="ml-auto flex flex-row gap-3">
                <button v-if="section.type !== 'text'" class="rounded border border-green-500 p-1 hover:bg-green-500 hover:text-white transition duration-300">Add {{ section.name }}</button>
                <button class="rounded border border-red-500 p-1 hover:bg-red-500 hover:text-white transition duration-300">Delete Section</button>
            </div>
        </div>
