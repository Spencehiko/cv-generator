    ...
    <div v-if="addDialog.show" class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-slate-900 text-white rounded-lg text-xl font-bold text-center p-10">
        <div>
            <div v-for="(input, index) in addDialog.inputs" :key="index" class="flex flex-row justify-between gap-5 my-5">
                ...
                <input v-if="input.type === 'input'" :id="input.name" class="w-60 bg-slate-500 border border-transparent pl-1 rounded outline-none hover:border-gray-400 focus:border-gray-800" />
                <textarea
        ...
        <div class="flex flex-row gap-5 justify-center mt-10">
            ...
            <button @click="addItem" class="border border-green-400 rounded p-2 text-green-400 transition-all duration-500 hover:bg-green-400 hover:text-slate-900">Yes</button>
        </div>
    </div>
</template>
