    ...
    <table class="w-full text-left mt-5 overflow-x-auto block">
        <thead>
            <tr>
                ...
                <th class="border p-3">Given By</th>
                <th class="border p-3">Certificate Name</th>
                <th class="border p-3">Given Date</th>
                <th class="border p-3">Certificate ID</th>
                <th class="border p-3">Certificate URL</th>
        ...
        <tbody>
            <tr v-for="(data, index) in sections[sectionIndex].data" :key="index">
                ...
                <td class="border p-3">{{ data.givenBy }}</td>
                <td class="border p-3">{{ data.name }}</td>
                <td class="border p-3">{{ (data.date.month + 1 < 10 ? "0" + (data.date.month + 1) : data.date.month + 1) + "/" + data.date.year }}</td>
                <td class="border p-3">{{ data.id }}</td>
