...
<template>
    <div class="flex flex-row flex-wrap justify-between">
        <draggable class="w-full cursor-move" tag="div" :list="sections[sectionIndex].data" item-key="info-drag">
            <template #item="{ index }">
                <div class="w-1/2 inline-flex items-center my-2 basis-1/2" :class="{ 'justify-end': index % 2 }">
                    ...
                    <input v-model="sections[sectionIndex].data[index].value" class="w-full-20 border border-transparent pl-1 rounded outline-none ml-5 hover:border-gray-400 focus:border-gray-800" />
